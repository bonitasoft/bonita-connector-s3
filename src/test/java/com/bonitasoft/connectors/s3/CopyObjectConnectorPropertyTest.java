...
class CopyObjectConnectorPropertyTest {
    ...
    Arbitrary<String> validRegions() {
        return Arbitraries.of("us-east-1", "us-west-2", "eu-west-1", "eu-central-1", "ca-central-1");
    }
    ...
    private Map<String, Object> validInputs() {
        ...
        inputs.put("sourceObjectKey", "source/key.txt");
        inputs.put("destinationBucketName", "dest-bucket");
        inputs.put("destinationObjectKey", "dest/key.txt");
        return inputs;
    }

    @Property
    void mandatorySourceBucketRejectsBlank(@ForAll("blankStrings") String bucket) {
        var connector = new CopyObjectConnector();
    ...
    @Property
    void mandatorySourceObjectKeyRejectsBlank(@ForAll("blankStrings") String key) {
        var connector = new CopyObjectConnector();
        var inputs = validInputs();
        inputs.put("sourceObjectKey", key);
        connector.setInputParameters(inputs);
        assertThatThrownBy(connector::validateInputParameters)
                .isInstanceOf(ConnectorValidationException.class);
    }
    ...
    void validConfigurationAlwaysBuilds(
            @ForAll @AlphaChars @StringLength(min = 1, max = 50) String srcKey,
            @ForAll @AlphaChars @StringLength(min = 1, max = 50) String destKey) {
        var connector = new CopyObjectConnector();
        var inputs = validInputs();
        inputs.put("sourceObjectKey", srcKey);
        inputs.put("destinationObjectKey", destKey);
        connector.setInputParameters(inputs);
    ...
    void bucketNameValidation(@ForAll("validBucketNames") String bucketName) {
        ...
        var connector = new CopyObjectConnector();
        var inputs = validInputs();
        inputs.put("sourceBucketName", bucketName);
        inputs.put("destinationBucketName", bucketName);
        connector.setInputParameters(inputs);
        assertThatCode(connector::validateInputParameters).doesNotThrowAnyException();
    }
    ...
    void objectKeyMaxLength(@ForAll @AlphaChars @StringLength(min = 1, max = 1024) String key) {
        ...
        var inputs = validInputs();
        inputs.put("sourceObjectKey", key);
        inputs.put("destinationObjectKey", key);
        connector.setInputParameters(inputs);
        assertThatCode(connector::validateInputParameters).doesNotThrowAnyException();
    }
