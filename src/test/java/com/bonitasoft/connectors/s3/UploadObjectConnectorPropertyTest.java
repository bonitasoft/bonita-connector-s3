...
class UploadObjectConnectorPropertyTest {
    ...
    Arbitrary<String> validRegions() {
        return Arbitraries.of("us-east-1", "us-west-2", "eu-west-1", "ap-southeast-1", "sa-east-1");
    }
    ...
    void validConfigurationAlwaysBuilds(
            @ForAll @AlphaChars @StringLength(min = 1, max = 50) String objectKey,
    ...
    void bucketNameValidation(@ForAll("validBucketNames") String bucketName) {
        Assume.that(bucketName.length() >= 3 && !bucketName.contains(".."));
        var connector = new UploadObjectConnector();
        var inputs = validInputs();
        inputs.put("bucketName", bucketName);
        connector.setInputParameters(inputs);
